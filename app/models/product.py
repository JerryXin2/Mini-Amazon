...
''',
    ...
    @staticmethod
    def get_reviews(prod_rev_id, available=True):
        rows = app.db.execute('''
    SELECT DISTINCT p.product_id, p.product_name, p.category, p.description, p.image, p.price, p.available, p.seller_id, p.quantity, s.seller, pr.review
    FROM Products as p, Product_Reviews as pr, Sellers as s
    WHERE p.product_id = pr.product_id AND p.product_id = :prod_rev_id 
    AND p.seller_id = s.uid
    GROUP BY p.product_id, s.seller, pr.review
    ''',
                                prod_rev_id = prod_rev_id,
                                available=available)
...
class P1:
    ...
    AND p.product_id = pr.product_id
    AND p.seller_id = s.uid
    GROUP BY p.product_id, s.seller
    ''',
   ...
   SELECT DISTINCT p.product_id, p.product_name, p.category, p.description, p.image, p.price, p.available, p.seller_id, p.quantity, AVG(pr.rating) as avg_rating, s.seller
    ...
    def filter_category(search_key, cat, sortPrice, available=True):
        ...
        SQL_str ='''SELECT DISTINCT p.product_id, p.product_name, p.category, p.description, p.image, p.price, p.available, p.seller_id, p.quantity, AVG(pr.rating) as avg_rating, s.seller
                    FROM Products as p, Product_Reviews as pr, Sellers as s
                    WHERE p.product_id = pr.product_id
                        AND p.category = :cat
                        AND p.seller_id = s.uid
                        AND p.product_name LIKE CONCAT('%', :search_key, '%')
                    GROUP BY p.product_id, s.seller
                    '''
        SQL_str = SQL_str + '\n' + order_by1
